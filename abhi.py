...
# FROM https://www.kaggle.com/suryavan/facebook-v-predicting-check-ins/knn-and-probability-lookup-script/discussion
#
...
weights = np.tile(np.array([490.0, 980.0, 4.0, 3.1, 2.1, 10.0, 10.0, 36])[
                  :, None], n_cell_x*n_cell_y).T  # feature weights
...
def makeprobtable(train, feature, threshold):
    ...
    # threshold small probabilities including zeros to threshold value
...
def getprob(ind, table, nn):
    ...
    # find indices that are not in lookup and set to zero
    temp[np.concatenate((temp1, temp2, temp3))] = 0
...
def get_map3():
    ...
    train = pd.read_csv(
        '../input/train.csv', dtype={'place_id': np.int64}, index_col=0)
    ...
    grid = extendgrid(
        extension_x, extension_y, size_x, size_y, n_cell_x, n_cell_y)
    ...
    for g_id in repeats:
        ...
        grid_train = train[
            (tr.x > xmin) & (tr.x < xmax) & (tr.y > ymin) & (tr.y < ymax)]
        ...
        indices[row_ids] = le.inverse_transform(
            np.argsort(y_pred, axis=1)[:, ::-1][:, :10])
    ...
    # for placeid = 999999999 and weekday = 5, then the index is wkday_ind =
    # 99999999905
    train['wkday_ind'] = 10*train['place_id'] + \
        np.floor(train['weekday']).astype(np.int64)
    train['hr_ind'] = 100*train['place_id'] + \
        np.floor(train['hour']).astype(np.int64)
    train['four_hour_ind'] = 100*train['place_id'] + \
        np.floor(train['four_hour']).astype(np.int64)
    ...
    wkday_indices = 10*indices + \
        np.tile(np.floor(test.weekday[:, None]).astype(np.int64), nn)
    hr_indices = 100*indices + \
        np.tile(np.floor(test.hour[:, None]).astype(np.int64), nn)
    four_hour_indices = 100*indices + \
        np.tile(np.floor(test.four_hour[:, None]).astype(np.int64), nn)
    ...
    if cross_validation == 1:
        ...
        # print('indices:', ([1/1.0, 1/2.0, 1/3.0]*(ytest[:, None] ==
        #       indices[:, 0:3])).sum()/indices[np.nonzero(indices[:, 0])].shape[0])
        # print('map3', ([1/1.0, 1/2.0, 1/3.0]*(ytest[:, None] == max3placeids[:, 0:3])
...
def get_vali_with_params(w_x=490.0, w_y=980.0, w_hour=4.0, w_weekday=3.1,
    ...
    w_knn_prob = knn_prob
    w_hour_prob = hour_prob
    w_weekday_prob = weekday_prob
