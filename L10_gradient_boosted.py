...
def main():
    ...
    for xlims, ylims in itertools.product(x_ranges, y_ranges):
        ...
        if i % 100 == 0:
            print("Updating gradient_boost_test_{}.csv".format(starttime))
            pd.concat(preds_test).to_csv(
                'gradient_boost_test_{}.csv'.format(starttime), index=False)
            pd.concat(preds_vali).to_csv(
                'gradient_boost_vali_{}.csv'.format(starttime), index=False)
