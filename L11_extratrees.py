...
def main():
    ...
    for xlims, ylims in itertools.product(x_ranges, y_ranges):
        ...
        clf_total = ExtraTreesClassifier(n_estimators=100, criterion='entropy',
        ...
        clf_trainfold  = ExtraTreesClassifier(n_estimators=100, criterion='entropy',
                                           random_state=10, n_jobs=-1)
        ...
        if i % 100 == 0:
            print("Updating extratrees_test_{}.csv".format(starttime))
            pd.concat(preds_test).to_csv(
                'extratrees_test_{}.csv'.format(starttime), index=False)
            pd.concat(preds_vali).to_csv(
                'extratrees_vali_{}.csv'.format(starttime), index=False)

        i += 1
    print("Writing out final extratrees_test_{}.csv".format(starttime))
    pd.concat(preds_test).to_csv('extratrees_test_{}.csv'.format(starttime), index=False)
    pd.concat(preds_vali).to_csv('extratrees_vali_{}.csv'.format(starttime), index=False)
