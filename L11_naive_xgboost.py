...
def main():
    ...
    for xlims, ylims in itertools.product(x_ranges, y_ranges):
        ...
        clf_total = xgboost.XGBClassifier(objective='multi:softprob',
            seed=10, subsample=0.5, learning_rate=0.075, max_depth=4,
            n_estimators=120, colsample_bytree=0.8)
    ...
    pd.concat(preds_test).to_csv('naive_xgboost_test_{}.csv'.format(starttime), index=False) 
    pd.concat(preds_vali).to_csv('naive_xgboost_vali_{}.csv'.format(starttime), index=False)
