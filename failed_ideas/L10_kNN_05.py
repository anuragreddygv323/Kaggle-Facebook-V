...
#  with NEIGH=5 set in environment
knn_opt_params = {
    'th': 3.8737,
    'w_x': 650.3430,
    'w_y': 1000.,
...
def main():
    ...
    for xlims, ylims in itertools.product(x_ranges, y_ranges):
        ...
        if i % 100 == 0:
            print("Updating knn_05_test_{}.csv".format(starttime))
            pd.concat(preds_test).to_csv(
                'knn_05_test_{}.csv'.format(starttime), index=False)
            pd.concat(preds_vali).to_csv(
                'knn_05_vali_{}.csv'.format(starttime), index=False)

        i += 1
    print("Writing out final knn_05_test_{}.csv".format(starttime))
    pd.concat(preds_test).to_csv('knn_05_test_{}.csv'.format(starttime), index=False)
    pd.concat(preds_vali).to_csv('knn_05_vali_{}.csv'.format(starttime), index=False)
    print("All done")
