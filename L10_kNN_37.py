...
#  with NEIGH=37 set in environment
knn_opt_params = {
    'th': 0.7346,
    'w_x': 484.0180,
    'w_y': 1000.,
...
def main():
    ...
    for xlims, ylims in itertools.product(x_ranges, y_ranges):
        ...
        if i % 100 == 0:
            print("Updating knn_37_test_{}.csv".format(starttime))
            pd.concat(preds_test).to_csv(
                'knn_37_test_{}.csv'.format(starttime), index=False)
            pd.concat(preds_vali).to_csv(
                'knn_37_vali_{}.csv'.format(starttime), index=False)
